...
import aggregate, { rawTileToIntArray } from '../../util/aggregate'
...
const SMALL_TILE_PATH = path.join(__dirname, 'mocks', 'carriers-2-3-2.pbf')
const BIG_TILE_PATH = path.join(__dirname, 'mocks', 'carriers-1-1-0.pbf')

beforeEach(() => {
  tileRaw = fs.readFileSync(SMALL_TILE_PATH)
...
test('Big tile, big delta, single frame mode, last feature', () => {
  const bigTileRaw = fs.readFileSync(BIG_TILE_PATH)
